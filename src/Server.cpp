...
#define WIFI_SSID "STRSO"
#define WIFI_PSK "1011040311037"
...
void handleGetEvents(HTTPRequest *req, HTTPResponse *res)
{
  ...
  JsonArray arr = jsonBuffer.createNestedArray("events");
  for (int i = 0; i < MAX_EVENTS; i++)
  {
    if (events[i].active)
    {
...
void handlePostEvent(HTTPRequest *req, HTTPResponse *res)
{
  ...
  // Parse the object
  // JsonObject &reqObj = JsonDocument.parseObject(buffer);
  DeserializationError reqObj = deserializeJson(jsonBuffer, buffer);
  ...
  if (eventID > -1)
  {
    ...
    StaticJsonDocument<JSON_OBJECT_SIZE(4)> resBuffer;

    // Create an object at the root
    JsonObject resObj = resBuffer.to<JsonObject>();
